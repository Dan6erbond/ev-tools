...
5. **Global Unit System & Settings Context**: All tools MUST consume unit preferences (distance, EV efficiency, gas consumption, fuel price, currency) directly from `SettingsContext` (`useSettings()`). Tools MUST NOT render local unit dropdown selectors in calculator cards. When new unit parameters are introduced, they MUST be added to `@/lib/types.ts`, `@/lib/units.ts`, `SettingsContext` (`@/components/settings-context.tsx`), and the Profile settings page (`@/app/profile/page.tsx`) supporting all standard metric, US imperial, and UK imperial options so unit preferences are saved globally across the app.
